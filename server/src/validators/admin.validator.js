...
// Password is permitted when an admin creates a teacher or student account.
const accountCreationForbiddenFields = forbiddenFields.filter(
...
function rejectUnexpected(fields) {
  return fields.map((field) =>
    body(field).custom((value) => {
      if (value !== undefined) {
        throw new Error(`${field} cannot be specified`);
      }
...
const createTeacherRules = () => [
  body('username')
    .trim()
    .matches(/^[a-z/offA-Z0-9_.-]{3,80}$/)
    .withMessage('Username must use 3-80 safe username characters'),
...
const statusRules = () => [
  ...
  ...rejectUnexpected(forbiddenFields),
];
...
const createDepartmentRules = () => [
  ...
  body('deptShortName')
    .trim()
    .notEmpty()
    .withMessage('Department short name is required')
    .isLength({ min: 2, max: 30 })
    .withMessage('Department short name must be between 2 and 30 characters'),
