...
function errorResponse(error, res) {
  const map = {
    ...
    INVALID_USERNAME: [400, 'Username is required'], INVALID_EMAIL: [400, 'A valid email is required'], INVALID_PASSWORD: [400, 'Password is required'],
    TEACHER_NAME_REQUIRED: [400, 'Teacher name is required'], TEACHER_DEPARTMENT_REQUIRED: [400, 'A valid teacher department ID is required'],
    USERNAME_TAKEN: [409, 'Username is already taken'], EMAIL_TAKEN: [409, 'Email is already registered'], DEPARTMENT_NOT_FOUND: [404, 'Department not found'],
  };
...
async function createUser(req, res) { try { const user = await adminService.createUser(req.body); return res.status(201).json({ success: true, data: { user } }); } catch (error) { return errorResponse(error, res); } }

module.exports = { list, getOne, updateStatus, updateRole, createUser };