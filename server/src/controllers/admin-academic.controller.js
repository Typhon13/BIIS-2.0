...
const listDepartments = action((req) => academicService.listDepartments());
const createDepartment = action((req) => academicService.createDepartment(req.body), 201);
const listCourses = action((req) => academicService.listCourses());
const createCourse = action((req) => academicService.createCourse(req.body), 201);
const listTerms = action((req) => academicService.listTerms());
const createTerm = action((req) => academicService.createTerm(req.body), 201);
const listTeachers = action((req) => academicService.listTeachers());
const listOfferings = action((req) => academicService.listOfferings());
