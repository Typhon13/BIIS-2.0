...
function identity(label) {
  const username =
    `phase2e_dept_${label}_${Date.now()}_` +
...
async function createAdmin(label = 'admin') {
  ...
  const created = await adminBootstrapService.createInitialAdmin({
    ...credentials,
    password,
...
async function createTeacher(userId, deptId, name = 'Teacher') {
  ...
  const response = await db.query(
    `INSERT INTO teachers (user_id, name, dept_id)
     VALUES ($1, $2, $3)
     RETURNING teacher_id, user_id, dept_id, name`,
    [
      userId,
      `${name} ${suffix}`,
      deptId,
...
async function cleanup() {
  if (teacherIds.length) {
    await db.query(
      'DELETE FROM teachers WHERE teacher_id = ANY($1::bigint[])',
      [teacherIds]
    );
  }
  ...
  if (departmentIds.length) {
    await db.query(
      'DELETE FROM departments WHERE dept_id = ANY($1::bigint[])',
      [departmentIds]
    );
  }
...
test('requires Admin authentication for department endpoints', async () => {
  ...
  const studentResponse = await request(app)
    ...
    .set(
      'Authorization',
      `Bearer ${studentLogin.body.data.accessToken}`
    )
    .set('x-role', 'ADMIN');
...
test('lists departments with pagination and search', async () => {
  ...
  const response = await request(app)
    .get('/api/admin/departments?page=1&limit=10&search=LIST')
  ...
  assert.ok(
    response.body.data.departments.some(
      (item) => String(item.deptId) === String(department.dept_id)
    )
  );

  assert.deepEqual(
    Object.keys(response.body.data.departments[0]).sort(),
    [
      'deptId',
      'deptName',
      'deptShortName',
...
test('creates and updates a department with valid head assignment', async () => {
  ...
  // Keep the name unique so repeated test runs cannot collide.
  const suffix = crypto.randomBytes(4).toString('hex');
  const deptName = `Applied Physics ${suffix}`;
  const deptShortName = `APHY${suffix}`;

  const created = await request(app)
    .post('/api/admin/departments')
    .set('Authorization', `Bearer ${token}`)
    .send({ deptName, deptShortName });

  assert.equal(created.status, 201);
  assert.equal(created.body.data.department.deptName, deptName);

  const createdDeptId = created.body.data.department.deptId;
  departmentIds.push(createdDeptId);
  ...
  const update = await request(app)
    ...
    .set('Authorization', `Bearer ${token}`)
    .send({ headId: teacher.teacher_id });

  assert.equal(update.status, 200);
  assert.equal(
    update.body.data.department.headId,
...
test('rejects department head assignment outside the same department', async () => {
  ...
  const invalid = await request(app)
    .patch(`/api/admin/departments/${departmentA.dept_id}`)
    .set('Authorization', `Bearer ${token}`)
    .send({ headId: teacher.teacher_id });

  assert.equal(invalid.status, 400);
  assert.match(invalid.body.message, /same department/i);
});