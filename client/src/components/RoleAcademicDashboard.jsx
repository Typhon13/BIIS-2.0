...
        <fieldset className="academic-prerequisites-fieldset">

          <legend>Prerequisites</legend>

          <p className="academic-field-help">

            Select every course that must be completed before a student can register for this course.

          </p>



          {!courses.filter((candidate) => candidate.courseId !== editingCourseId).length ? (

            <span>No other courses are available yet.</span>

          ) : (

            <div className="academic-prerequisite-list">

              {courses

                .filter((candidate) => candidate.courseId !== editingCourseId)

                .map((candidate) => {

                  const candidateId = String(candidate.courseId)

                  const checked = courseForm.prerequisites.includes(candidateId)



                  return (

                    <label key={candidate.courseId} className="academic-prerequisite-option">
...
                              ? current.prerequisites.filter((id) => id !== candidateId)

                              : [...current.prerequisites, candidateId],
...
                          .map((id) => courses.find((candidate) => String(candidate.courseId) === String(id))?.code || `Course #${id}`)
